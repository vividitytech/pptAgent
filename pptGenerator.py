...
class PPTGenerator(DataBase):
    ...
    def chat2sql_call_llm(self, user_message, system_message, cursor, db, args):
        ...
        for i in range(args.max_tries):
            try:
                # sql_script = vn_openai.submit_prompt([vn_openai.system_message("You are expert in SQLite and you can geenrate SQlite script based on user guidence (for example: user query sentence) and database schema. The generated script should output necessary columm names. You have the follow tables and you need to generate SQLlite script which can run \n" + infos), vn_openai.user_message("what are the top 10 customers by sales")])
                #df = vn_openai.run_sql(sql_script)
